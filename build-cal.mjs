...
/* שמות ליומן. אימון משולב מסומן 1/2 ו-2/2 כדי שלא ייראה ככפילות בטלפון. */
const SHORT= {bikeLong:'רכיבה ארוכה',runBrick:'Brick אחרי הרכיבה',runBrick2:'ריצת מעבר',
  runLong:'ריצה ארוכה',bikeKey:'אופניים איכות',bikeEnd:'אופניים נפח',bikeSpin:'ספין שחרור',
  swimA:'שחייה סטים',swimB:'שחיית ים',swimB2:'שחיית ים',swimC:'שחייה קלה',runEasy:'ריצה קלה',
  str:'כוח 1/2 · רגליים כבד',str2:'כוח 2/2 · ליבה וירכיים',mob:'ניידות',race:'מרוץ'};
...
/* «כוח 1/2» ו-«2/2» רק כששני החלקים באמת מתקיימים באותו יום.
   אימון כוח יחיד נקרא «כוח» — אחרת השם מבטיח חלק שני שאינו קיים. */
const PAIRNM = {str:['כוח 1/2 · רגליים כבד','כוח · רגליים כבד'],
                str2:['כוח 2/2 · ליבה וירכיים','כוח · ליבה וירכיים']};
const shortName = (s, dayRoles) => {
  if(s.role==='race')
    return strip(s.title).replace(/^מרוץ הכנה · /,'').replace(/ · יום ראשון.*$/,'');
  if(PAIRNM[s.role]){
    const other = s.role==='str' ? 'str2' : 'str';
    const paired = dayRoles ? dayRoles.includes(other) : true;
    return PAIRNM[s.role][paired ? 0 : 1];
  }
  return SHORT[s.role]||strip(s.title).replace(/^[^·]+· /,'');
};
...
for (const w of API.WEEKS) {
  ...
  days.forEach((D,di)=>{
    ...
    const dayRoles=items.map(x=>x.s.role);
    const seen={};
    items.forEach((it,ix)=>{
      const s=it.s, f=API.fuelFor(s,w), rv=API.recoveryFor(s,w);
      const k=(seen[s.role]=(seen[s.role]||0)+1), suff=k>1?'-'+k:'';
      const cont=!!s.after, P=[];
      /* המספר של האימון עצמו בא ראשון ומסומן במפורש. סך היום מופיע אחריו
         ומסומן כ"היום כולו" — בלי זה השניים נקראים כסתירה. */
      P.push(`◆ האימון הזה: ${HM(s.dur)} · RPE ${s.rpe}${s.dist&&s.dist!=='—'?' · '+strip(s.dist):''}${s.where?' · '+strip(s.where):''}`);
      P.push(`היום כולו: ${blocks===1?'אימון אחד':blocks+' אימונים'} · ${HM(dayMin)} סה"כ`);
      P.push(`שבוע ${w.n}/52 · ${API.PH_META[w.ph].n} · ${ctx.t}${w.rec?' · שבוע התאוששות':''}${w.race?' · שבוע מרוץ':w.prep?' · שבוע עם מרוץ הכנה':''}`);
      /* שני חלקי אימון משולב מתארים זה את זה, עם המספרים בפועל */
      let cn=null; try{ cn=API.comboNote(w.n,s); }catch(e){}
      if(cn) P.push('◆ '+strip(cn.text));
      else if(cont) P.push(`◆ החלק השני של אימון משולב — ${s.after.gap?`${Math.round(s.after.gap*60)} דקות`:'מיד'} אחרי ${RLBL[s.after.role]||s.after.role}.`);
      P.push('');
      if(LITE){
      ...
      const ev=['BEGIN:VEVENT',
        ...
        fold(`SUMMARY:${ICON[s.t]} ${cont?'↳ ':''}${esc(shortName(s,dayRoles))} · ${HM(s.dur)}`),
      ...
      if(!cont&&s.t!=='mob') ev.push('BEGIN:VALARM','TRIGGER:-PT45M','ACTION:DISPLAY',
        fold(`DESCRIPTION:${esc('בעוד 45 דק׳: '+shortName(s,dayRoles)+' · '+HM(s.dur))}`),'END:VALARM');
